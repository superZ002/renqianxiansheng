document.addEventListener('DOMContentLoaded', function() {
    ...
    var quoteLibrary = {
        sunset: {
            object: '夕阳', emotion: '绚烂', occasion: '黄昏',
        ...
        mountain: {
            object: '山川', emotion: '壮阔', occasion: '登高',
        ...
        food: {
            object: '美食', emotion: '满足', occasion: '聚餐',
        ...
        flower: {
            object: '繁花', emotion: '美好', occasion: '春日',
        ...
        rain: {
            object: '细雨', emotion: '静谧', occasion: '雨天',
        ...
        night: {
            object: '星空', emotion: '深邃', occasion: '深夜',
    ...
    function saveSettings() {
        try {
            localStorage.setItem('rxiansheng_settings', JSON.stringify(settings));
        } catch (e) {}
    }
    ...
    function callVLM() {
        ...
        if (settings.model === 'doubao') {
            callDoubao(userPrompt, imageBase64);
        } else {
            callGLM(userPrompt, imageBase64);
        }
    }
    ...
    function buildPrompt() {
        ...
        prompt += '### 1. 古风雅韵\n';
        prompt += '- 风格：古典诗词或文言风韵\n';
        prompt += '- 要求：有意境、有画面感，用词典雅\n';
        prompt += '- 参考标准：接近唐诗宋词的审美，但不必拘泥于格律\n';
        prompt += '- 字数：不超过20字\n\n';

        prompt += '### 2. 现代文学\n';
        prompt += '- 风格：散文诗或现代文学金句\n';
        prompt += '- 要求：有哲思或情感共鸣，语言精炼有力\n';
        prompt += '- 参考风格：类似鲁迅、张爱玲、村上春树、余华等作家的语言质感\n';
        prompt += '- 字数：不超过25字\n\n';
        ...
        prompt += '  * 禁止单纯玩烂梗或生硬吐槽\n';
        prompt += '- 参考示例：\n';
        prompt += '  * "天上在开股东大会，乌云是反对派，光是最后的决议。"\n';
        prompt += '  * "云层密谋已久，却被一束光出卖了。"\n';
        prompt += '- 字数：不超过20字\n\n';
        ...
        prompt += '- 方言辨识度示例对比：\n';
        prompt += '  四川话版本：\n';
        prompt += '  ❌ 错误示范："这云层翻涌，巴适得板"\n';
        prompt += '  （问题：普通话句式+2个方言词点缀，辨识度弱）\n';
        prompt += '  ✅ 正确示范："天老爷把云朵和得巴巴适适的，光从中间戳的眼眼飚出来，硬是安逸"\n';
        prompt += '  （特点：方言句式+方言词汇+方言语气词）\n\n';
        prompt += '  东北话版本：\n';
        prompt += '  ❌ 错误示范："这景色真美，老带劲了"\n';
        prompt += '  ✅ 正确示范："这云彩整得跟棉裤腰似的，老天爷搁中间儿开了个天窗，敞亮！"\n\n';
        prompt += '  粤语版本：\n';
        prompt += '  ❌ 错误示范："个天好靓，好犀利"\n';
        prompt += '  ✅ 正确示范："啲云厚到遮天，但係光都係要打条路出嚟，世界终归係光嘅"\n\n';
        prompt += '  河南话版本：\n';
        prompt += '  ❌ 错误示范："这风景真得劲儿"\n';
        prompt += '  ✅ 正确示范："老天爷把云彩堆得跟山样，中间扒开一道缝，光就淌下来了，得劲儿！"\n\n';
        prompt += '  上海话版本：\n';
        prompt += '  ❌ 错误示范："这个天老灵额"\n';
        prompt += '  ✅ 正确示范："迭个云厚是厚得唻，但光总归要寻着缝钻出来额，嗲是嗲得唻"\n\n';

        prompt += '## 地区选择逻辑\n';
        prompt += '用户选择的地区：' + region + '\n';
        prompt += '请根据用户选择的地区，使用对应的方言文化来表达"地方韵味"风格。\n';
        prompt += '如果用户未选择或选择了"北方"，使用"北方方言通用语"。\n\n';

        prompt += '## 输出格式\n';
        prompt += '请严格按照以下JSON格式返回，不要包含任何额外文字：\n';
        prompt += '{\n';
        prompt += '  "gufeng": "古风文案内容",\n';
        prompt += '  "xiandai": "现代文学文案内容",\n';
        prompt += '  "humor": "冷幽默文案内容",\n';
        prompt += '  "difang": "地方韵味文案内容（使用用户选择的地区方言）"\n';
        prompt += '}\n\n';
    ...
    function parseAndShowResult(content) {
        ...
        if (!result) {
            ...
            lines.forEach(function(line) {
                if (line.indexOf('主体') !== -1 || line.indexOf('object') !== -1) {
                    result.object = line.replace(/.*[:：]/, '').trim();
                } else if (line.indexOf('氛围') !== -1 || line.indexOf('情绪') !== -1 || line.indexOf('emotion') !== -1) {
                    result.emotion = line.replace(/.*[:：]/, '').trim();
                } else if (line.indexOf('意境') !== -1 || line.indexOf('场合') !== -1 || line.indexOf('occasion') !== -1) {
                    result.occasion = line.replace(/.*[:：]/, '').trim();
                } else if (line.indexOf('古风') !== -1 || line.indexOf('gufeng') !== -1) {
                    result.gufeng = line.replace(/.*[:：]/, '').trim();
                } else if (line.indexOf('现代') !== -1 || line.indexOf('xiandai') !== -1) {
                    result.xiandai = line.replace(/.*[:：]/, '').trim();
                } else if (line.indexOf('幽默') !== -1 || line.indexOf('冷幽默') !== -1 || line.indexOf('humor') !== -1) {
                    result.humor = line.replace(/.*[:：]/, '').trim();
                } else if (line.indexOf('地方') !== -1 || line.indexOf('韵味') !== -1 || line.indexOf('difang') !== -1) {
                    result.difang = line.replace(/.*[:：]/, '').trim();
                }
            });
        }
        ...
        var tagsSection = document.querySelector('.shenshi-tags');
        if (tagsSection) {
        ...
        if (settings.styles.classic !== false && (result.gufeng || result.classic)) {
            ...
            document.querySelector('.style-classic').style.display = 'none';
        }
        ...
        if (settings.styles.modern !== false && (result.xiandai || result.modern)) {
            ...
            document.querySelector('.style-modern').style.display = 'none';
        }

        if (settings.styles.humor !== false && (result.humor || result.humor)) {
            resultHumor.textContent = result.humor || '';
            document.querySelector('.style-humor').style.display = '';
        } else {
            document.querySelector('.style-humor').style.display = 'none';
        }
        ...
        if (settings.styles.difang !== false && result.difang) {
            ...
            document.querySelector('.style-difang').style.display = 'none';
        }
    ...
    function classifyImage() {
        ...
        try {
            model.classify(previewImage, function(error, results) {
                ...
                if (category && quoteLibrary[category]) {
                    showResult(quoteLibrary[category]);
                } else {
                    showMysteryResult();
                }
            });
    ...
    function matchTextCategory(text) {
        ...
        var keywords = {
            'sunset': ['夕阳', '晚霞', '黄昏', '落日', '日落', 'sunset'],
            'mountain': ['山', '爬山', '登顶', '高峰', '山川', 'mountain'],
            'sea': ['海', '大海', '海边', '沙滩', '海浪', 'sea'],
            'city': ['城市', '夜景', '加班', '工作', '打工人', 'city'],
            'food': ['吃', '美食', '饭', '火锅', '烧烤', 'food'],
            'flower': ['花', '春天', '花园', 'flower'],
            'rain': ['雨', '下雨', '雨天', 'rain'],
            'night': ['夜', '星空', '月亮', '熬夜', '失眠', 'night']
        };
        for (var cat in keywords) {
            for (var i = 0; i < keywords[cat].length; i++) {
                if (lower.indexOf(keywords[cat][i]) !== -1) {
                    return cat;
                }
            }
        }
    ...
    function initCopyButtons() {
        copyBtns.forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                switch (style) {
                    case 'classic':
                        text = resultClassic.textContent;
                        break;
                    case 'modern':
                        text = resultModern.textContent;
                        break;
                    case 'humor':
                        text = resultHumor.textContent;
                        break;
                    case 'difang':
                        text = resultDifang.textContent;
                        break;
                }
                if (text && text !== '-' && text !== '——') {
    ...
    function initShare() {
        shareBtn.addEventListener('click', function() {
            var parts = [];
            if (settings.styles.classic !== false && resultClassic.textContent) {
                parts.push('【古风】' + resultClassic.textContent);
            }
            if (settings.styles.modern !== false && resultModern.textContent) {
                parts.push('【现代】' + resultModern.textContent);
            }
            if (settings.styles.humor !== false && resultHumor.textContent) {
                parts.push('【吐槽】' + resultHumor.textContent);
            }
            if (settings.styles.difang !== false && resultDifang.textContent) {
