document.addEventListener('DOMContentLoaded', function() {
    ...
    function showUploadPicker() {
        ...
        var isMiui = ua.indexOf('MiuiBrowser') > -1 || ua.indexOf('XiaoMi') > -1;
        if (!isMobile) {
            fileInput.removeAttribute('capture');
            fileInput.click();
            return;
        }
        var albumText = isMiui ? '选择文件' : '从相册选择';
        var albumHint = isMiui ? '<div style="text-align:center;padding:8px 16px 0;font-size:12px;color:#999;">提示：在 DCIM/图片 文件夹中找到照片</div>' : '';
        var picker = document.createElement('div');
        picker.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:flex-end;justify-content:center;';
        picker.innerHTML = '<div style="background:#fff;border-radius:16px 16px 0 0;width:100%;max-width:420px;padding:8px 0 16px;animation:slideUp 0.25s ease;">'
            + '<div style="text-align:center;padding:12px 0 8px;font-size:13px;color:#999;font-family:serif;">选择上传方式</div>'
            + '<button id="pickCamera" style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:16px;border:none;background:transparent;font-size:16px;color:#b53a2f;font-family:serif;cursor:pointer;border-bottom:1px solid #f0f0f0;">'
            + '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>'
            + '拍照</button>'
            + '<button id="pickAlbum" style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:16px;border:none;background:transparent;font-size:16px;color:#5a6a4a;font-family:serif;cursor:pointer;border-bottom:1px solid #f0f0f0;">'
            + '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>'
            + albumText + '</button>'
            + albumHint
            + '<button id="pickCancel" style="display:flex;align-items:center;justify-content:center;width:100%;padding:16px;border:none;background:transparent;font-size:16px;color:#999;font-family:serif;cursor:pointer;">取消</button>'
            + '</div>';
        document.body.appendChild(picker);
        var styleEl = document.createElement('style');
        styleEl.textContent = '@keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}';
        document.head.appendChild(styleEl);
        picker.querySelector('#pickCamera').addEventListener('click', function() {
            document.body.removeChild(picker);
            fileInput.setAttribute('capture', 'environment');
            fileInput.accept = 'image/*';
            fileInput.click();
        });
        picker.querySelector('#pickAlbum').addEventListener('click', function() {
            document.body.removeChild(picker);
            fileInput.removeAttribute('capture');
    ...
    function buildPrompt() {
        ...
        prompt += '  * 保留方言的"语气感"——比如四川话的"嘛""噻""咯"，东北话的"整""搁""老"\n';
    ...
    function initVisitCounter() {
        fetch('https://api.countapi.xyz/get/renqianxiansheng/v1', { mode: 'cors', cache: 'no-cache' }).then(function(res) {
            return res.json();
        }).then(function(data) {
            var count = parseInt(data.value || '2847', 10);
            visitCountEl.textContent = count;
        }).catch(function() {
            var localCount = parseInt(localStorage.getItem('xiansheng_count') || '2847', 10);
            visitCountEl.textContent = localCount;
        });
    }

    function incrementVisitCounter() {
        fetch('https://api.countapi.xyz/hit/renqianxiansheng/v1', { mode: 'cors', cache: 'no-cache' }).then(function(res) {
            return res.json();
        }).then(function(data) {
            var count = parseInt(data.value || '2847', 10);
            visitCountEl.textContent = count;
            ...
            var localCount = parseInt(localStorage.getItem('xiansheng_count') || '2847', 10);
            localCount++;
            localStorage.setItem('xiansheng_count', String(localCount));
            visitCountEl.textContent = localCount;
        });
    }
});